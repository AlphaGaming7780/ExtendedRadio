...
namespace ExtendedRadio.Systems
{
    public partial class MainSystem : GameSystemBase
    {
        ...
        {
            if (_pauseRadioBinding.WasPerformedThisFrame() && ExtendedRadio.radio != null && ExtendedRadio.radio.isEnabled) _radioUISystemTraverse.Method("SetPaused", new[] { typeof(bool) }).GetValue(!ExtendedRadio.radio.paused);
            if (_muteRadioBinding.WasPerformedThisFrame() && ExtendedRadio.radio != null && ExtendedRadio.radio.isEnabled) _radioUISystemTraverse.Method("SetMuted", new[] { typeof(bool) }).GetValue(!ExtendedRadio.radio.muted);
            ...
            if (_volumeUpRadioBinding.WasPerformedThisFrame() && ExtendedRadio.radio != null && ExtendedRadio.radio.isEnabled) _radioUISystemTraverse.Method("SetVolume", new[] { typeof(float) }).GetValue(SharedSettings.instance.audio.radioVolume + 0.05f);
            if (_volumeDownRadioBinding.WasPerformedThisFrame() && ExtendedRadio.radio != null && ExtendedRadio.radio.isEnabled) _radioUISystemTraverse.Method("SetVolume", new[] { typeof(float) }).GetValue(SharedSettings.instance.audio.radioVolume - 0.05f);
        }
    }
}